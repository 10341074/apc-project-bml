...
CSR::CSR(std::vector<Car> * p): Sparse(p) {}
CSR::CSR(std::unique_ptr< std::vector<Car> > & p): Sparse(p) {}
CSR::CSR(std::size_t nr, std::size_t nc, std::size_t nz, std::vector<Car> * p): Sparse(nr,nc,nz,p) {}
CSR::CSR(std::size_t nr, std::size_t nc, std::size_t nz, std::unique_ptr< std::vector<Car> > & p): Sparse(nr,nc,nz,p) {}
...
void CSR::ostream(std::ostream & os) const{
...
//	std::vector<Color>::const_iterator 			itA=mat.V->A.begin();
	for(std::size_t k=0; k < this->nr; ++k){
		if(it->I == k){
			...
			for(int spaces = this->nc; spaces>1; --spaces)
				os << White << SEPARATOR;
			os << White << '\n';
		}
	}
...
void CSR::push_back(const std::vector<Color> & vec){
	std::size_t col=0;
...
Color CSR::operator()(const std::size_t i, const std::size_t j) const {
...
void CSR::move_line(std::size_t line, Color cl){
...
void CSR::transpose(){
	...
	P=std::vector<std::size_t>(nr+1,0);
	std::size_t car=0;
